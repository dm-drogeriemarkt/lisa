...
const SubnetSelectInput = ({ onChange, ...attrs }) => {
  const {
    updateAttribute,
    attributes: {
      appTierName,
  ...
  const handleChange = ({ subnetId }) => {
    ...
    onChange({ subnetId, vlanid })
  }

  return (
    <SelectInput value={subnetId} options={subnets} onChange={handleChange} disabled={!appTierName} loading={loading} {...attrs} />
  )
}
